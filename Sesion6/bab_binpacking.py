...
def bin_packing_solver(objects, C):
    class BinPacking_BaB_PS(BabPartialSolution):
        ...
        def calc_opt_bound(self) -> "int or float":
            ...
            list_cont = list(self.container_weights)
            ...
            if self.n == len(objects): ##Si estamos en una hoja
                return len(list_cont)
            restante = sum(objects[len(self.decisions):])
            for num_con, container_weight in enumerate(self.container_weights):
                if C - container_weight >= min_object:
                    #list_cont[num_con] +=  C - container_weight
                    restante -= C - container_weight
            n_cont = ceil(restante/C)
            #print("opt: ",len(self.container_weights)+n_cont)
            return len(self.container_weights)+n_cont
        ...
        def calc_pes_bound(self) -> "int or float":
            lista_cont = list(self.container_weights)
            
            for obj in objects[len(self.decisions):]:
                for i in range(len(lista_cont)):
                    if lista_cont[i] + obj <= C:
                        lista_cont[i] += obj    
                        break
                else:
                    lista_cont.append(obj)                    
            #print("pes: ",len(lista_cont))
            return len(lista_cont)
