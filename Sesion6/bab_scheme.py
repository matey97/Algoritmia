...
class BabSolver():   
    @staticmethod
    def solveMaximization(initial_ps : "BabPartialSolution") -> "Solution":
        heap = MaxHeap()
        heap.add(initial_ps) 
        bps = initial_ps.pes()
        while len(heap)>0:
            ...
            for new_ps in best_ps.successors():
                ...
                if new_ps.opt() >= bps:
                    heap.add(new_ps) 
    @staticmethod
    def solveMinimization(initial_ps : "BabPartialSolution") -> "Solution":
        heap = MinHeap() 
        heap.add(initial_ps) 
        bps = initial_ps.pes()
        while len(heap)>0:
